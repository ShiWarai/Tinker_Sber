...
from tinker_msgs.msg import ControlCmd, LowCmd, LowState, MotorCmd, MotorState, OneMotorCmd
...
def _quaternion_to_rpy(x: float, y: float, z: float, w: float):
...
class MotorSliderNode(Node):
    def __init__(self, motor_count: int):
        ...
        self.imu_subscriber = self.create_subscription(
            Imu, "/imu_state", self.imu_callback, 10
        )
    ...
    def imu_callback(self, msg: Imu):
        """Callback for IMU data on /imu_state (unified stack)."""
...
class SliderWindow(QWidget):
    ...
    def _build_motor_group(self, motor_index: int) -> QGroupBox:
        # Display motor number starting from 1 instead of 0
    ...
    def _send_control_cmd(self, cmd_value: int):
        ...
        motor_id_display = self.control_motor_id.value()
        motor_id_internal = motor_id_display - 1
        
        self.ros_node.publish_control_command(motor_id_internal, cmd_value)
    ...
    def _on_timer(self):
        ...
        if self.ros_node.has_imu:
            imu = self.ros_node.current_imu
            q = imu.orientation
            quat_str = f"[{q.x:.3f}, {q.y:.3f}, {q.z:.3f}, {q.w:.3f}]"
            roll, pitch, yaw = _quaternion_to_rpy(q.x, q.y, q.z, q.w)
            rpy_str = f"[{math.degrees(roll):.1f}°, {math.degrees(pitch):.1f}°, {math.degrees(yaw):.1f}°]"
