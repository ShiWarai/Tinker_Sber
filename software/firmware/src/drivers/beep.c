...
u8 music_sel;
//  11  21
// ����(0~3)����(0~7)   ||   STOPѭ������(0~7) ��ֹ������(0~7)
#define B1 13
#define B2 16
#define B3 15
u8 start_music_px4[] = {
    B1, 12, B2, 12, B3, 12, B1, 12, B2, 12, B3, 12, B1, 12, B2, 12, B3, 12,
    B1, 12, B2, 12, B3, 12, B2, 12, B3, 11, B2, 11, B3, 11, B2, 11, B3, 43,
};

u8 start_music_pi[] = {27, 01, 00, 01, 00, 59, 00, 59};

u8 start_music_pi_thread[] = {27, 01, 00, 01, 17, 01, 00, 01, 00, 59, 00, 59};

u8 start_music_windows[] = {
    21, 21, 01, 11, 21, 21, 01, 11, 21, 21, 01, 11, 11, 13, 15, 13, 13, 11, 01, 11, 13, 11, 01,
    11, 13, 11, 01, 11, 21, 23, 15, 41, 01, 11, 15, 31, 01, 11, 15, 21, 01, 11, 15, 11, 01, 11,
};
...
u8 mems_gps_music[] = {15, 44, 17, 33, 17, 33};

u8 mems_right_music[] = {05, 64, 00, 13, 12, 64};
...
u8 bat_error_music[] = {
    07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33,
    00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32,
    07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32, 07, 33, 00, 32,
};

u8 rc_error_music[] = {27, 11, 00, 11, 27, 11, 00, 11, 27, 11, 00, 11, 00, 39, 00, 39};

u8 hml_cal_music[] = {13, 11, 00, 11, 13, 11, 00, 11, 13, 11, 00, 11, 00, 39, 00, 39};

u8 bldc_cal_zero_music[] = {05, 64, 00, 13, 12, 64};

u8 bldc_cal_init_music[] = {03, 64, 00, 13, 15, 34};

u8 bldc_reset_err_music[] = {
    02, 34, 00, 32, 02, 34, 00, 32,
};

u8 gait_switch_music[] = {15, 14, 00, 13, 12, 14};

u8 start_music_pix[] = {
    04, 32, 24, 22, 05, 32, 04, 32, 24, 22, 05, 32, 04, 32, 24, 22, 05, 32,
};

u8 dj_cal_music1[] = {27, 01, 00, 01, 00, 39, 00, 39};
u8 dj_cal_music2[] = {27, 01, 00, 01, 27, 01, 00, 01, 00, 39, 00, 39};
u8 dj_cal_music3[] = {27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 00, 39, 00, 39};
u8 dj_cal_music4[] = {27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 00, 39, 00, 39};
u8 dj_cal_music5[] = {27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 00, 39, 00, 39};
u8 dj_cal_music6[] = {27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01,
                      00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 00, 39, 00, 39};
u8 dj_cal_music7[] = {27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01,
                      27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 00, 39, 00, 39};
u8 dj_cal_music8[] = {27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01,
                      00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 27, 01, 00, 01, 00, 39, 00, 39};
...
u8 Play_Music_In_Task(u8 *music, u16 st, u16 ed, u8 en, float dt)
{
	...
	case 1:
		if (en)
		{
			...
			if (beep_state_task_cnt++ > Beat_delay[beat] * loop / 1000. / dt)
			{
				Tone(level, tone);
				i++;
				beep_state_task_cnt = 0;
...
char Play_Music_Task(u8 sel, float dt) //<-----------------
{
...
#endif
		break;
	case RC_ERO_BEEP:; // Play_Music_In_Task(rc_error_music,0,sizeof(rc_error_music)/2,1,dt);
		break;
	case BEEP_STATE:
	...
	case BEEP_DJ_CAL1:
		...
		{
		case 0:
			finish = Play_Music_In_Task(dj_cal_music1, 0, sizeof(dj_cal_music1) / 2, 1, dt);
			break;
		case 1:
			finish = Play_Music_In_Task(dj_cal_music2, 0, sizeof(dj_cal_music2) / 2, 1, dt);
			break;
		case 2:
			finish = Play_Music_In_Task(dj_cal_music3, 0, sizeof(dj_cal_music3) / 2, 1, dt);
			break;
		case 3:
			finish = Play_Music_In_Task(dj_cal_music4, 0, sizeof(dj_cal_music4) / 2, 1, dt);
			break;
		case 4:
			finish = Play_Music_In_Task(dj_cal_music5, 0, sizeof(dj_cal_music5) / 2, 1, dt);
			break;
		case 5:
			finish = Play_Music_In_Task(dj_cal_music6, 0, sizeof(dj_cal_music6) / 2, 1, dt);
			break;
		case 6:
			finish = Play_Music_In_Task(dj_cal_music7, 0, sizeof(dj_cal_music7) / 2, 1, dt);
			break;
		case 7:
			finish = Play_Music_In_Task(dj_cal_music8, 0, sizeof(dj_cal_music8) / 2, 1, dt);
			break;
		}

		break;
	case BEEP_BLDC_ZERO_CAL:
		finish = Play_Music_In_Task(bldc_cal_zero_music, 0, sizeof(bldc_cal_zero_music) / 2, 1, dt);
		break;
	case BEEP_BLDC_ZERO_INIT:
		finish = Play_Music_In_Task(bldc_cal_init_music, 0, sizeof(bldc_cal_init_music) / 2, 1, dt);
		break;
	case BEEP_BLDC_GAIT_SWITCH:
		finish = Play_Music_In_Task(gait_switch_music, 0, sizeof(gait_switch_music) / 2, 1, dt);
		break;
	case BEEP_BLDC_RESET_ERR:
		finish = Play_Music_In_Task(bldc_reset_err_music, 0, sizeof(bldc_reset_err_music) / 2, 1, dt);
		break;
...
u16 tone_table[3][8] = {{0, 261, 293, 329, 349, 391, 440, 493},
