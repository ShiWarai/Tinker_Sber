...
int icm20602_gyro_calibrate_step(s16 gx, s16 gy, s16 gz, float *out_ox, float *out_oy, float *out_oz)
{
	...
	{
		if (out_ox)
			*out_ox = (float)icm_gyro_sum[0] / ICM_CAL_SAMPLES;
		if (out_oy)
			*out_oy = (float)icm_gyro_sum[1] / ICM_CAL_SAMPLES;
		if (out_oz)
			*out_oz = (float)icm_gyro_sum[2] / ICM_CAL_SAMPLES;
		icm_gyro_cal_cnt = 0;
...
int icm20602_accel_calibrate_step(s16 ax, s16 ay, s16 az, float *out_ox, float *out_oy, float *out_oz)
{
	...
	{
		if (out_ox)
			*out_ox = (float)icm_acc_sum[0] / ICM_CAL_SAMPLES;
		if (out_oy)
			*out_oy = (float)icm_acc_sum[1] / ICM_CAL_SAMPLES;
		if (out_oz)
			*out_oz = (float)icm_acc_sum[2] / ICM_CAL_SAMPLES;
		icm_acc_cal_cnt = 0;
...
float converRT(float xn, float yn, float zn, s16 *xb, s16 *yb, s16 *zb)
{
	...
	RT[0][0] = cosd(-pit);
	RT[0][1] = sind(-pit) * sind(-rol);
	RT[0][2] = sind(-pit) * cosd(-rol);
	RT[1][0] = 0;
	RT[1][1] = cosd(-rol);
	RT[1][2] = -sind(-rol);
	RT[2][0] = -sind(-pit);
	RT[2][1] = cosd(-pit) * sind(-rol);
	RT[2][2] = cosd(-pit) * cosd(-rol);
...
{
	icm20602_write_reg(ICM20_PWR_MGMT_1, 0x80); //??��????��??x41,??????
	Delay_ms(1000);
...
uint8_t icm20602_get_accel_adc(void)
{
	...
	if (abs(data_acc.x) < 6000 && abs(data_acc.y) < 6000 && abs(data_acc.z) < 6000)
	{
		...
		lis3mdl.Acc_I16.z = data_acc.z;
#endif
		converRT(lis3mdl.Acc_I16.x, lis3mdl.Acc_I16.y, lis3mdl.Acc_I16.z, &lis3mdl_cov.Acc_I16.x,
		         &lis3mdl_cov.Acc_I16.y, &lis3mdl_cov.Acc_I16.z);
	}
	return 0;
}
...
uint8_t icm20602_get_gyro_adc(void)
{
	...
	if (abs(data_g.x) < 6000 && abs(data_g.y) < 6000 && abs(data_g.z) < 6000)
	{
		...
		lis3mdl.Gyro_I16.z = data_g.z;
#endif
		converRT(lis3mdl.Gyro_I16.x, lis3mdl.Gyro_I16.y, lis3mdl.Gyro_I16.z, &lis3mdl_cov.Gyro_I16.x,
		         &lis3mdl_cov.Gyro_I16.y, &lis3mdl_cov.Gyro_I16.z);
	}
	return 0;
}
