...
{ 
  ...
  DMA_DeInit(DMA_Streamx);
	...
	while (DMA_GetCmdStatus(DMA_Streamx) != DISABLE){}// ждём, пока DMA можно настроить
...
#define DMA_Priority_VeryHigh             ((uint32_t)0x00030000)*/
	switch(pri){
		case 0:		 DMA_InitStructure.DMA_Priority = DMA_Priority_Low;// низкий приоритет
		break;
		case 1:		 DMA_InitStructure.DMA_Priority = DMA_Priority_Medium;// средний
		break;
		case 2:		 DMA_InitStructure.DMA_Priority = DMA_Priority_High;// высокий
		break;
		case 3:		 DMA_InitStructure.DMA_Priority = DMA_Priority_VeryHigh;// очень высокий
		break;
	}
...
void MYDMA_Enable(DMA_Stream_TypeDef *DMA_Streamx,u16 ndtr)
{
	...
	while (DMA_GetCmdStatus(DMA_Streamx) != DISABLE){}	// ждать отключения
