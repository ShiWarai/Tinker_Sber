...
u8 mit_motor_mode_en( char id,char en){//ʹ�ܵ��
	u8 mbox;
  u16 i=0;
	CanTxMsg TxMessage;
	if(id<5)
...
char data_can_mit_send(motor_measure_t *ptr){//���Ϳ���ָ��
	...
	float t_ff = fminf_mit(fmaxf_mit(T_MIN_CAN_MIT[ptr->param.id],LIMIT(set_t,-ptr->max_t,ptr->max_t)), T_MAX_CAN_MIT[ptr->param.id]); //���շ��͵�����ǰ��
	t_des_ff[ptr->param.id]=t_ff;
	if(EN_MIT_PID_INNER==0||ptr->param.usb_cmd_mode==2)//������ǰ��������ģʽ
	   kp=kd=v_des=0;
	
	if(ptr->param.control_mode==1){//ת��ģʽ7
		kp=0;
